...
function getStringLength(value) {
  if (value != null && typeof value !== 'undefined') return value.length;
  return 0;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if ('aeiouy'.indexOf(lowerCaseStr[i]) !== -1) {
      vowelCount += 1;
    }
  }
  return vowelCount;
...
function invertCase(str) {
  ...
  const invertedStr = characters.join('');
  return invertedStr;
}
...
function extractNameFromTemplate(value) {
  ...
  const name = value.slice(startIndex, -1).trim();
  return name;
}
...
function encodeToRot13(str) {
  ...
  return str
    .replace(
      /[a-z]/g,
      (match) => letters[1][(letters[1].indexOf(match) + 13) % 26]
    )
    .replace(
      /[A-Z]/g,
      (match) => letters[0][(letters[0].indexOf(match) + 13) % 26]
    );
}
